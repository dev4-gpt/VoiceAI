...
export const EvalsDashboard: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/60 rounded-2xl border border-slate-800/80 backdrop-blur-xl p-5 shadow-2xl space-y-5 relative overflow-hidden">
      {/* Running Radar Overlay */}
      {isRunning && (
        <div className="absolute inset-0 z-30 bg-slate-950/80 backdrop-blur-sm flex flex-col items-center justify-center space-y-4 animate-in fade-in">
          <div className="relative flex items-center justify-center">
            <div className="w-24 h-24 rounded-full border border-purple-500/30 animate-ping absolute" />
            <div className="w-16 h-16 rounded-full border border-cyan-500/40 animate-pulse absolute" />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-4">
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className="px-2 py-0.5 rounded-full bg-purple-500/10 border border-purple-500/30 text-purple-400 text-[10px] font-mono font-bold">
        ...
        <button
          ...
          disabled={isRunning}
          className="flex items-center space-x-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 via-indigo-600 to-cyan-600 hover:from-purple-500 hover:to-cyan-500 text-white font-semibold text-xs shadow-lg shadow-purple-900/30 transition-all disabled:opacity-50"
        >
          {isRunning ? (
            <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
        {/* Metric 1: pass@5 */}
        <div className="p-3.5 rounded-xl bg-slate-950/80 border border-slate-800 relative overflow-hidden shadow-md">
        ...
        <div className="p-3.5 rounded-xl bg-slate-950/80 border border-cyan-500/30 relative overflow-hidden shadow-md group">
          {/* Subtle Sparkline SVG in Background */}
          <svg className="absolute bottom-1 right-2 w-28 h-10 opacity-30 text-cyan-400" viewBox="0 0 100 30">
            <path
          ...
          <div className="text-slate-400 text-xs font-mono flex items-center justify-between">
            ...
            <span className="text-[9px] font-mono text-cyan-300 bg-cyan-950/50 px-1 rounded border border-cyan-800/40">TREND ↗</span>
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-950/80 border border-amber-500/30 relative overflow-hidden shadow-md group">
          {/* Subtle Sparkline SVG in Background */}
          <svg className="absolute bottom-1 right-2 w-28 h-10 opacity-30 text-amber-400" viewBox="0 0 100 30">
            <path
          ...
          <div className="text-slate-400 text-xs font-mono flex items-center justify-between">
            ...
            <span className="text-[9px] font-mono text-emerald-300 bg-emerald-950/50 px-1 rounded border border-emerald-800/40">STABLE</span>
          </div>
      ...
      <div className="flex-1 space-y-3 overflow-y-auto custom-scrollbar pr-1">
        <div className="flex items-center justify-between text-xs font-mono uppercase tracking-wider text-slate-400">
          ...
          <span className="text-purple-400 text-[11px]">Click task to inspect all 5 trials</span>
        </div>
        ...
        {tasks.map((task: EvalTask) => {
          ...
          return (
            <div
              key={task.id}
              className="rounded-xl bg-slate-950/70 border border-slate-800 hover:border-slate-700 transition-all overflow-hidden"
            >
              ...
              >
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    <span className="font-semibold text-xs text-slate-200">{task.name}</span>
                    <span className="text-[10px] font-mono text-slate-500">(5 trials executed)</span>
                  </div>
                  <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                    {task.graders.split('|').map((g: string, gIdx: number) => (
                      <span
                        key={gIdx}
                        className="px-2 py-0.5 rounded bg-emerald-950/60 border border-emerald-800/50 text-emerald-400 font-mono text-[10px] font-semibold"
                      >
                        {g.trim()}
                      </span>
                    ))}
                ...
                <div className="flex items-center space-x-3 font-mono">
                  <span className="text-emerald-400 bg-emerald-950/40 px-2 py-1 rounded border border-emerald-900/40 text-xs">
                    pass@5: {task.passRate}%
                  </span>
                  <span className="text-purple-400 bg-purple-950/40 px-2 py-1 rounded border border-purple-900/40 text-xs">
                    pass^5: {task.pass5}%
                  </span>
              ...
              {isExpanded && (
                <div className="border-t border-slate-800 bg-slate-950/90 p-4 space-y-3 animate-in fade-in duration-150">
                  <div className="text-[11px] font-mono text-slate-400 flex items-center justify-between">
                    <span>Trial Logs &amp; Deterministic Execution Traces:</span>
                    <span className="text-cyan-400">All 5 Trials Inspected</span>
                  </div>
                  ...
                  <div className="space-y-2">
                    {task.trials.map((trial) => (
                      <div
                        ...
                        className="p-3 rounded-lg bg-slate-900/90 border border-slate-800 space-y-1.5 font-mono text-xs"
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            <span className="px-1.5 py-0.5 rounded bg-purple-950 border border-purple-800/60 text-purple-300 font-bold text-[10px]">
                            ...
                            <span className="text-slate-400 text-[11px]">{trial.latencyMs}ms TTFA</span>
                          </div>
                        ...
                        <div className="text-slate-300 text-[11px]">
                          <span className="text-slate-500 font-semibold">User Input:</span> "{trial.inputSnippet}"
                        </div>

                        <div className="text-slate-300 text-[11px] bg-black/50 p-2 rounded border border-slate-800/80">
                          <span className="text-indigo-400 font-semibold">Agent Reasoning Trace:</span> {trial.agentTrace}
                        </div>
                      </div>
                    ))}
                  </div>

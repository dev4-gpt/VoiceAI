...
| Owner-only demo routes (content trigger/audit/dispatch/jobs approve/reject/publish/auto-pipeline, compliance log, all Instatic routes, billing usage/record-call) | `ORCHESTRATOR_API_KEY` set | **503 `ADMIN_UNCONFIGURED` in every environment, including local dev, when unset.** The voice agent's CRM tools, the CRM board and checkout stay open without the key, as before. |

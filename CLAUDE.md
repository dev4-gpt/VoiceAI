# 🤖 CLAUDE.md — StratosGTM Project Guidelines & Context

Welcome, Claude! This file provides the essential context, commands, architecture, and coding guidelines for working with **StratosGTM**.

---

## 🌟 What This Project Is

**StratosGTM** is an autonomous, voice-first growth operating system built for founders, creators, agencies, and enterprise operators. Powered by **AssemblyAI's Voice Agent API**, it:
...
| TryPost publishing (`POST /api/me/publish`, `GET /api/me/trypost/accounts`) | Signed in, the workspace saved its TryPost API token under Keys, `TRYPOST_BASE_URL` set, and `ENABLE_REAL_PUBLISHING=true` | 503 `TRYPOST_UNCONFIGURED` without the base URL, 409 `TRYPOST_NOT_CONNECTED` without a token; with real publishing off, a labeled simulated receipt (`isSimulated: true`). A receipt is real only after TryPost's `GET /api/posts/{id}` reports the post `published`; accepted-but-unconfirmed is `queued` and still `isSimulated: true`. Publishing is two calls, verified live: `POST /api/posts` always creates a draft (even with `scheduled_at`), then `PUT /api/posts/{id}` with `status: "publishing"` publishes. **Proven end to end on Bluesky on 2026-09-26** (a post typed in the app appeared on the account). Only Bluesky is enabled on the TryPost instance (every other platform is switched off to fit its 1 GB memory cap); Instagram and other media-first networks need platform app registration inside TryPost and media support, and are refused for text-only posts. A receipt can still read `queued` if TryPost is slower than the short check window. Instance setup and failure modes: `docs/superpowers/specs/2026-09-26-trypost-operations.md`. |
...
When the user asks you to plug their business into StratosGTM:

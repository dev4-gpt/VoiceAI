#!/usr/bin/env node
// scripts/apply-buyerlab2-schema.cjs
// Applies apps/orchestrator/src/db/sql/buyerlab2.sql in one transaction.
//   node scripts/apply-buyerlab2-schema.cjs --dry-run   list statements and the target host only
//   node scripts/apply-buyerlab2-schema.cjs             apply
...
async function main() {
  ...
  const file = path.resolve(__dirname, '../apps/orchestrator/src/db/sql/buyerlab2.sql');
  const statements = fs
